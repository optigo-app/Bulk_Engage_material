            ...
            return (
              <div key={bag.rfbag || idx} className="sum-bag-row">
                ...
                <span className="sum-bag-row__material" style={{ color }}>
                  {getMaterialIcon(bag.material, 13)}
                  {bag.material || '—'}
                </span>

                {/* Detail: shape · quality · size */}
                <span className="sum-bag-row__detail">
                  {[bag.shape, bag.quality, bag.size].filter(Boolean).join(' · ') || bag.desc || '—'}
                </span>
        ...
        <div className="summary__breakdown">
          ...
          <div className="summary__breakdown-grid">
            {Object.entries(byMaterial).map(([mat, data]) => {
              ...
              return (
                <div key={mat} className="summary__breakdown-card" style={{ '--mat-color': color }}>
                  <div className="summary__breakdown-icon" style={{ color }}>
                    {getMaterialIcon(mat, 20)}
                  </div>
                  <div className="summary__breakdown-info">
                    <span className="summary__breakdown-name">{mat}</span>
                    <div className="summary__breakdown-vals">
                      <span>{data.count} bag{data.count !== 1 ? 's' : ''}</span>
                      <span>{data.pcs} pcs</span>
                      <span>{data.wt.toFixed(3)} ct</span>
                    </div>
                  </div>
                </div>
              );
